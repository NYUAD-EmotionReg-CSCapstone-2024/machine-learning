...
from braindecode.models.modules import MaxNormLinear, CausalConv1d

class ModifiedATCNet(nn.Module):
    def __init__(
            self,
            n_chans=512,
            timestamps=124, 
        ...
        super().__init__()
        self.n_chans, self.timesteps = n_chans, timestamps
        ...
        # Define shapes for attention and TCN blocks
        self.F2 = self.n_chans  # Use the channel dimension from EEGPT
        self.Tc = self.timesteps
        ...
        self.temporal_conv_nets = nn.ModuleList([
            nn.Sequential(
                _TCNResidualBlock(
                    in_channels=self.F2,  # First block gets 512 channels
                ...
                *[_TCNResidualBlock(
                    in_channels=tcn_n_filters,  # Subsequent blocks get 32 channels from the first Conv (512 -> 32)
                    kernel_size=tcn_kernel_size,
                    n_filters=tcn_n_filters,
                    dropout=tcn_dropout,
                    activation=tcn_activation,
                    dilation=2 ** i
                ) for i in range(1, tcn_depth)]
            ) for _ in range(self.n_windows)
        ])
        ...
        if self.concat:
            self.final_layer = nn.ModuleList([
                MaxNormLinear(
                    in_features=tcn_n_filters * self.n_windows,  # Use 32 as input dimension
            ...
            self.final_layer = nn.ModuleList([
                MaxNormLinear(
                    in_features=tcn_n_filters,  # Use 32 as input dimension
                    out_features=self.n_outputs,
                    max_norm_val=self.max_norm_const
                ) for _ in range(self.n_windows)
            ])
    ...
    def forward(self, x):
        # Input x is from EEGPT: [batch_size, F2, Tc]
        # No need for ensure4d and dimshuffle as in original ATCNet
        
        # Sliding window processing
        sw_concat = []
        for w in range(self.n_windows):
            # Extract window
            conv_feat_w = x[..., w:w + self.Tw]
            ...
            # Apply final layer if not concatenating
            if not self.concat:
...
class _TCNResidualBlock(nn.Module):
    ...
    def forward(self, X):
        ...
        # Process the input through convolution layers
        out = self.conv1(X)
        out = self.bn1(out)
        ...
        # Apply reshaping_conv to the ORIGINAL INPUT X, not to 'out'
        residual = self.reshaping_conv(X)
