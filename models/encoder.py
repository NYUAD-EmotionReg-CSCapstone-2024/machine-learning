...
# # create a class called PreTrainedEncoder that inherits from the EEGTransformer class
# # and loads the pre-trained weights from the checkpoint file
# class PreTrainedEncoder(EEGTransformer):
...
# class LinearProbeEEGPT(nn.Module):
#     def __init__(self, ckpt_path, num_classes, freeze_base=True):
#         super().__init__()
#         self.base_model = PreTrainedEncoder(
#             ckpt_path,
#             img_size=[64, 1024],
#             patch_size=64,
#             embed_num=4,
...
#         with torch.no_grad():
#             dummy_input = torch.randn(1, *self.base_model.input_shape)
...
#     encoder = LinearProbeEEGPT(ckpt_path, num_classes=5)

#     sample_eeg = torch.randn(2, 64, 1000)
#     out = encoder(sample_eeg)
#     print(out.shape)
...
class FoundationClassifier(nn.Module):
    ...
    def __init__(self, num_classes, n_channels):
        ...
        self.model = EEGPTClassifier(
            ...
            img_size=[n_channels, 2000],
            use_channels_names=use_channel_names,
            use_chan_conv=True,
            use_predictor=True
        )

        # load self.model.target_encoder with weights
        ckpt_path = './EEGPT/checkpoint/eegpt_mcae_58chs_4s_large4E.ckpt'
        ...
        for k,v in ckpt['state_dict'].items():
            if k.startswith("target_encoder."):
                target_encoder_state[k[21:]]=v
        
        self.model.target_encoder.load_state_dict(target_encoder_state)
